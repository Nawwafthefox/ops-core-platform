-- Fix: memberships.role is enum role_type, but RPC receives text.
-- Cast text to public.role_type explicitly.
...
begin
  ...
  -- Cast to enum (this will throw if value not in enum)
  begin
  ...
  -- Detect user identifier column in memberships: prefer "user_id" else "profile_id"
  if exists (
    select 1
    from information_schema.columns
  ...
  using p_department_id, v_role_enum, p_company_id, p_target_user_id;
  ...
  using p_company_id, p_target_user_id, p_department_id, v_role_enum;
